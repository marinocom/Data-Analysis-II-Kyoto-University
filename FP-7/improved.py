...
#--------------------------------
# Helper Functions 
def display_title(s, num):
    display(Markdown(f"### Improved Analysis {num}: {s}"))
...
def analyze_feature_importance_improved(df):
    display_title('Improved Feature Engineering & Importance', 2)
...
def compare_models_improved(df):
    ...
    """
    display_title('Model Comparison with Dimensionality Reduction', 4)
